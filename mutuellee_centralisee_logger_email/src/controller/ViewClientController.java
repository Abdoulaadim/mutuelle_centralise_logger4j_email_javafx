...
public class ViewClientController implements Initializable {
    ...
    static  Logger logger = Logger.getLogger(ViewClientController.class.getName());
    ...
    if (checkinput()) {
    	...
    	if (i==1) {
    		...
    		emails.emails("Insertion", "ajout a été effectue avec succées", email.getText());
    		logger.info("ajout a ete effectue avec succee");
    		
		} else {
			MethodeAlert methodeAlert =new MethodeAlert();
    		methodeAlert.afficherAlert("Information Dialog", "Erreur d'ajout ");
    		emails.emails("Insertion", "Error d'ajout", email.getText());
